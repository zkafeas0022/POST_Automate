...
***Test Cases***
Add value Cash with Discount Coupon in Value
    Post Setup
    login POST Application
    Verify that all function of Fare card sale page    ${InitialPurse}    ${IssuingFee}    ${CardDeposit}
    ${Total_Value_Discount}    Cash with Discount Coupon in Value add value and confrim transaction    ${Initial_Add_Value}
    Cash with Discount Coupon in Value Verify Total payment    ${Total_Value_Discount}
    close Application
...
Cash with Discount Coupon in Value Verify Total payment
    [Arguments]    ${Total_Value_Discount}
...
${Project_Name}    Devlop
${Excelname}    Data_Fare_card_sale_Cash_Card_with_Discount_Coupon_in_Value.xlsx