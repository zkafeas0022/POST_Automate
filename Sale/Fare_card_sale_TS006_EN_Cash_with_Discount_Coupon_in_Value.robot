...
***Test Cases***
EN Cash with Discount Couponin Value
    ...
    \    ${ExecuteTestResult}	Run Keyword And Ignore Error    EN Cash with Discount Couponin Value    ${iData}
...
EN Cash with Discount Couponin Value
    ...
    ${Total_Value_Discount}    Cash with Discount Coupon in Value add value and confrim transaction    ${Initial_Add_Value}    ${iData}    ${Cash_Amount_Received}    ${CardDeposit}    ${Discount_Coupon}    ${Voucher_No}
    Cash with Discount Coupon in Value Verify Total payment    ${Total_Value_Discount}    ${Initial_Add_Value}    ${CardDeposit}    ${Discount_Coupon}    ${Voucher_No}
...
Cash with Discount Coupon in Value add value and confrim transaction
    [Arguments]    ${Initial_Add_Value}    ${iData}    ${Cash_Amount_Received}    ${CardDeposit}    ${Discount_Coupon}    ${Voucher_No}
    Attach Window	id:MainWindow	
    ...
    ${Total_Add_Value}    Evaluate    (${Initial_Add_Value}+${CardDeposit})    #200+50=250
    ${Total_Value_Discount}    Evaluate    (${Initial_Add_Value}+${CardDeposit})-${Discount_Coupon}    #(200+50)-40=210
    ${Amount_Change}    Evaluate    (${Total_Add_Value}-${Cash_Amount_Received})-${Discount_Coupon}  #500-(250-40)=290    200
    ${Cash_Amount_Received}    evaluate    ('{:,.2f}'.format(${Cash_Amount_Received}))
    ${Total_Value_Discount}    evaluate    ('{:,.2f}'.format(${Total_Value_Discount}))
    ${Amount_Change}    evaluate    ('{:,.2f}'.format(${Amount_Change}))
    ${Amount_Change}    Remove String    ${Amount_Change}    -
    Verify Text In Textbox	id:ControlPayment_TotalAmountReceived_TextBox    ${Cash_Amount_Received}
    Verify Text In Textbox	id:ControlPayment_TotalPayment_TextBox    ${Total_Value_Discount}
    Verify Text In Textbox    id:ControlPayment_ChangeDue_TextBox    ${Amount_Change}
    Click Button    id:ControlMainCardOperation_Confirm_button
    Return From Keyword    ${Total_Value_Discount}


Cash with Discount Coupon in Value Verify Total payment
    [Arguments]    ${Total_Value_Discount}    ${Initial_Add_Value}    ${CardDeposit}    ${Discount_Coupon}    ${Voucher_No}
    ...
    Item Should Be Disabled    id:ControlPaymentMode_Payment_combobox
    Verify Text In Textbox    id:ControlPaymentMode_Cash_textbox    ${Total_Value_Discount}
    ...
    ${InitialPurse}    evaluate    ${Total_Value_Discount}-${CardDeposit}
...
${Project_Name}    Devlop
${Excelname}    Data_Fare_card_sale_TS006_EN_Cash_with_Discount_Coupon_in_Value.xlsx