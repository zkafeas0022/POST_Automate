...
EN Cash with Cash Voucher
    ...
    \    ${ExecuteTestResult}	Run Keyword And Ignore Error    EN Cash with Cash Voucher    ${iData}
...
EN Cash with Cash Voucher
    ...
    Cash with Cash Voucher add value and confrim transaction    ${Initial_Add_Value}    ${CardDeposit}    ${Voucher}    ${Voucher_No}    ${iData}    ${Cash_Amount_Received}
    Cash with Cash Voucher Verify Total payment    ${Initial_Add_Value}    ${CardDeposit}    ${Voucher}    ${Voucher_No}
...
Cash with Cash Voucher add value and confrim transaction
    [Arguments]    ${Initial_Add_Value}    ${CardDeposit}    ${Voucher}    ${Voucher_No}    ${iData}    ${Cash_Amount_Received}    
    Attach Window	id:MainWindow	
 ...
 #Run keyword If    '${Cash_Amount_Received_Input}'!='${EMPTY}'    Input Text To Textbox    id:ControlCashAmountReceive_CashReceived_textbox    ${Cash_Amount_Received_Input}
    ...
    ${Change_Due}    Evaluate    ${Total_Initial_Add_Value}-${Total_Vocher_with_received}
...
Cash with Cash Voucher Verify Total payment
    [Arguments]    ${Initial_Add_Value}    ${CardDeposit}    ${Voucher}    ${Voucher_No}
...
${Project_Name}    Devlop
${Excelname}    Data_Fare_card_sale_TS004_EN_Cash_with_Cash_Voucher.xlsx