...
***Test Cases***
EN Credit Card with Cash Voucher
    ...
    \    ${ExecuteTestResult}	Run Keyword And Ignore Error    EN Credit Card with Cash Voucher    ${iData}
    \    ${Total_Addvalue}    Evaluate    ${Initial_Add_Value}+${Total_Addvalue} 
    log    ${Total_Addvalue}
    Close Application
...
EN Credit Card with Cash Voucher
    ...
    ${Initial_Add_Value}    Get Value Of Test Data Request By Name	${iData}	Initial_Add_Value
    ${CardDeposit}    Get Value Of Test Data Request By Name	${iData}	CardDeposit
    ${Voucher_No}    Get Value Of Test Data Request By Name	${iData}	Voucher_No
    ${Voucher}    Get Value Of Test Data Request By Name	${iData}	Voucher
    ${Credit_Caed_No}    Get Value Of Test Data Request By Name	${iData}	Credit_Caed_No
    Verify that all function of Fare card sale page    ${CardDeposit}
    ${Amount_credit_Card}    Credit card with Cash Voucher add value and confrim transaction    ${Initial_Add_Value}    ${iData}    ${Credit_Caed_No}    ${Voucher}    ${Voucher_No}    ${CardDeposit}
    Credit card with Cash Voucher Verify Total payment    ${Amount_credit_Card}    ${Initial_Add_Value}    ${CardDeposit}    ${Voucher}    ${Credit_Caed_No}    ${Voucher}    ${Voucher_No}
...
Credit card with Cash Voucher add value and confrim transaction
    [Arguments]    ${Initial_Add_Value}    ${iData}    ${Credit_Caed_No}    ${Voucher}    ${Voucher_No}    ${CardDeposit}
    Attach Window	id:MainWindow	
    ...
    Input Text To Textbox    id:ControlPaymentMode_CreditCardNo_textbox    ${Credit_Caed_No}
   ...
   #Run keyword If    '${Cash_Amount_Received_Input}'!='${EMPTY}'    Input Text To Textbox    id:ControlCashAmountReceive_CashReceived_textbox    ${Cash_Amount_Received_Input}
    ...
    ${Amount_credit_Card}    Evaluate    (${Initial_Add_Value}-${Voucher})+${CardDeposit}
    ${Total_Initial_With_Voucher}    Evaluate    ${Voucher}+${Amount_credit_Card}  
    ${Change_Due}    Evaluate    ${Total_Initial_Add_Value}-${Total_Initial_With_Voucher}
    ${Change_Due}    Convert To String     ${Change_Due} 
    ${Change_Due}    Remove String    ${Change_Due}    -
    ${Total_Initial_With_Voucher}    evaluate    ('{:,.2f}'.format(${Total_Initial_With_Voucher}))
    ${Total_Initial_Add_Value}    evaluate    ('{:,.2f}'.format(${Total_Initial_Add_Value}))
    ${Change_Due}    evaluate    ('{:,.2f}'.format(${Change_Due}))
    Verify Text In Textbox	id:ControlPayment_TotalAmountReceived_TextBox    ${Total_Initial_With_Voucher}
    ...
    Return From Keyword    ${Amount_credit_Card}
  
Credit card with Cash Voucher Verify Total payment
    [Arguments]    ${Amount_credit_Card}    ${Initial_Add_Value}    ${CardDeposit}    ${Voucher}    ${Credit_Caed_No}    ${Voucher}    ${Voucher_No}
    ...
    ${Amount_credit_Card}    evaluate    ('{:,.2f}'.format(${Amount_credit_Card}))
    Item Should Be Disabled    id:ControlPaymentMode_Payment_combobox
    Verify Text In Textbox    id:ControlPaymentMode_Cash_textbox    ${zero_baht}
    Verify Text In Textbox    id:ControlPaymentMode_CreditCard_textbox     ${Amount_credit_Card}
    Verify Text In Textbox    id:ControlPaymentMode_CreditCardNo_textbox    ${Credit_Caed_No}
    ...
    Set Global Variable    ${Initial_Add_Value}
...
${Project_Name}    Devlop
${Excelname}    Data_Fare_card_sale_TS007_EN_Credit_Card_with_Cash_Voucher.xlsx