...
***Test Cases***
Add Value Credit Card With Cash Voucher have Change Due

    Post Setup
    login POST Application
    Verify that all function of Fare card sale page    ${InitialPurse}    ${IssuingFee}    ${CardDeposit}
    ${Amount_credit_Card}    Credit card with Cash Voucher add value and confrim transaction    ${Initial_Add_Value}
    Credit card with Cash Voucher Verify Total payment    ${Amount_credit_Card}
    close Application
...
Credit card with Cash Voucher add value and confrim transaction
   ...
   #Run keyword If    '${Cash_Amount_Received_Input}'!='${EMPTY}'    Input Text To Textbox    id:ControlCashAmountReceive_CashReceived_textbox    ${Cash_Amount_Received_Input}
    ...
    ${Change_Due}    Run keyword If    '${Total_Initial_Add_Value}'=='${Total_Initial_With_Voucher}'    Set Variable    0    ELSE IF    '${Total_Initial_Add_Value}'!='${Total_Initial_With_Voucher}'    Evaluate    ${Total_Initial_Add_Value}-${Total_Initial_With_Voucher}
...
${Project_Name}    Devlop
${Excelname}    Data_Data_Add_Value_Credit_Card_With_Cash_Voucher_have_Change_Due.xlsx