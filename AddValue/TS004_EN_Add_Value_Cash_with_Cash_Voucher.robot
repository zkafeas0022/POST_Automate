...
***Test Cases***
Cash with Cash Voucher
    Post Setup
    ${Total_Addvalue}    set Variable    0
    login POST Application
    :FOR	${iData}	IN    @{TestDataRequest}
    \    Setup Test Data	
    \    ${ExecuteTestResult}	Run Keyword And Ignore Error	Cash with Cash Voucher    ${iData}
...
Cash with Cash Voucher
    ...
    Cash with Cash Voucher add value and confrim transaction    ${Add_Value}    ${iData}    ${Cash_Amount_Received}    ${Cash_Amount_Received_Input}    ${Voucher}    ${Voucher_No}
    Cash with Cash Voucher Verify Total payment    ${Add_Value}    ${Cash_Amount_Received}    ${Voucher}    ${Voucher_No}
    log    ${PurseAddValue}
...
Cash with Cash Voucher add value and confrim transaction
    [Arguments]    ${Add_Value}    ${iData}    ${Cash_Amount_Received}    ${Cash_Amount_Received_Input}    ${Voucher}    ${Voucher_No}
    Attach Window	id:MainWindow	
...
Cash with Cash Voucher Verify Total payment
    [Arguments]    ${Add_Value}    ${Cash_Amount_Received}    ${Voucher}    ${Voucher_No}
    ...
    Verify Text In Textbox    id:ControlBonusInformationB_BonusAfterCardSale_textbox    ${zero_baht}
    #Payment
    ${AmountReceived}    Evaluate    ${Add_Value}+${Voucher}
    ${AmountReceived}    Evaluate    ('{:,.2f}'.format(${AmountReceived}))
    Verify Text In Textbox	id:ControlPayment_TotalAmountReceived_TextBox	${AmountReceived}
    Verify Text In Textbox	id:ControlPayment_TotalPayment_TextBox	${Add_Value}
    Verify Text In Textbox	id:ControlPayment_ChangeDue_TextBox	${Voucher}
   #Button
...
${Excelname}    Data_AddValue_TS004_EN_Cash_with_Cash_Voucher.xlsx